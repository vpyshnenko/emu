# 🐦 Emu  
### *Песочница сетевых автоматов для асинхронных конечных автоматов*

Emu — это лёгкий рантайм для построения, соединения и запуска **сетей асинхронных конечных автоматов (AFSM)**.  
Он предоставляет простой и выразительный способ:

- описывать **локальное поведение** узлов  
- соединять их в **каузальную сеть**  
- запускать систему с **детерминированным событийным исполнением**

Emu — это не симулятор.  
Каждый узел работает на **реальной виртуальной машине**, а слой передачи сообщений — **заменяемый**.  
По умолчанию используется общая очередь FIFO — идеально подходит для обучения и экспериментов.

---

## 🚧 Область применения: только AFSM

Emu специально поддерживает **только асинхронные конечные автоматы**:

- ✔ событийное исполнение  
- ✔ детерминизм  
- ✔ конечное состояние  
- ✔ явная топология  
- ✔ каузальное поведение  

Emu **не поддерживает**:

- ✘ синхронные автоматы  
- ✘ автоматы с таймингом  
- ✘ недетерминированные переходы  
- ✘ параллелизм и интерливинг  

Такое ограничение делает Emu простым, предсказуемым и удобным для анализа.

---

# 🏗️ Emu как реальный рантайм

Каждый узел в Emu содержит **реальную детерминированную виртуальную машину**:

- стек  
- набор инструкций  
- постоянное состояние  
- строгая семантика  

Виртуальная машина не симулируется — это полноценный исполнительный механизм.

Единственный заменяемый компонент — **транспортный слой**.  
По умолчанию — общая очередь FIFO, но её можно заменить на:

- шину сообщений  
- аппаратные прерывания  
- распределённую систему  
- пользовательский планировщик  

Логика узлов и VM остаются неизменными.

---

# 🎓 Образовательный маршрут

Рекомендуемый путь изучения Emu:

### **1 — Запустите первую сеть**  
Используйте пример с Фибоначчи ниже.

### **2 — Узнайте, что такое узел**  
Узел содержит:
- локальное состояние  
- обработчики (программы VM)  
- выходные порты  

### **3 — Поймите топологию**  
Связи определяют, как события передаются между узлами.

### **4 — Изучите инструкции VM**  
Команды вроде `Load`, `Add`, `Emit`, `Halt`.

### **5 — Постройте свою мини-сеть**  
Начните с двух узлов, передающих числа друг другу.

### **6 — Исследуйте поведение**  
Циклы, счётчики, обратная связь, конвейеры.

---

## 🧩 Описание примера

Эта сеть состоит из трёх асинхронных конечных автоматов, каждый из которых работает на стековой виртуальной машине Emu.

### **NodeA и NodeB — стековые сумматоры по модулю**

NodeA и NodeB — это **стековые автоматы**, вычисляющие числа Фибоначчи по модулю.  
Каждый узел хранит два значения:

- текущее число Фибоначчи  
- верхнюю границу (модуль)

При получении события:

1. VM загружает операнды из стека  
2. Выполняет сложение по модулю (`AddMod`)  
3. Проверяет переполнение  
4. Отправляет:
   - обычное событие с результатом, или  
   - событие переполнения  

После вычисления каждый узел **передаёт результат соседу** через NodeC.  
Таким образом, A и B поочерёдно генерируют последовательность Фибоначчи по модулю.

### **NodeC — оркестратор и контроллер безопасности**

NodeC — не сумматор.  
Он выполняет две функции:

1. **Маршрутизация событий**  
   - Получает значения от A и отправляет их B  
   - Получает значения от B и отправляет их A  
   - Выводит последовательность через порт `out`

2. **Контроль исполнения**  
   - Имеет внутренний счётчик шагов  
   - Останавливает сеть при переполнении в A или B

NodeC — это диспетчер и предохранитель сети.

### **Запуск вычислений**

Сеть активируется **событием bang**, отправленным в **NodeB**, с начальным значением `1`.  
Далее цикл A → C → B → C → A генерирует последовательность Фибоначчи по модулю.

---

## 🌀 Пример: Сеть Фибоначчи по модулю

Пример из `test_fib_mod.ml` строит сеть AFSM, генерирующую числа Фибоначчи по модулю.

![Fibonacci Network Diagram](images/fib_net.png)  
*Диаграмма: каузальный поток между узлами A, B и C в модульной сети Фибоначчи.*

Узлы A и B вычисляют значения; узел C пересылает их и формирует выходную последовательность.

## 🧩 Описание примера

Эта сеть состоит из трёх асинхронных конечных автоматов, каждый из которых работает на внутренней стековой виртуальной машине Emu.

### **NodeA и NodeB — стековые сумматоры по модулю**

NodeA и NodeB — это **стековые AFSM‑автоматы**, вычисляющие числа Фибоначчи по модулю заданного предела.  
Каждый узел хранит два внутренних состояния:

- текущее число Фибоначчи  
- верхнюю границу (модуль)  

Когда один из узлов получает событие:

1. Виртуальная машина загружает операнды из стека  
2. Выполняет модульное сложение (`AddMod`)  
3. Проверяет, произошло ли переполнение  
4. Генерирует либо:
   - **обычное событие** с результатом, либо  
   - **событие переполнения**  

После вычисления нового значения каждый узел **передаёт результат соседу** через NodeC.  
Вместе A и B образуют каузальный цикл, генерирующий последовательность Фибоначчи по модулю `ceil`.

### **NodeC — оркестратор и контроллер безопасности**

NodeC не является сумматором.  
Он выполняет роль **оркестратора** и имеет две ключевые задачи:

1. **Пересылка событий**  
   - Получает значения от A и отправляет их B  
   - Получает значения от B и отправляет их A  
   - Выдаёт результирующую последовательность через порт `out`  

2. **Контроль исполнения**  
   - Поддерживает внутренний счётчик шагов, ограничивающий длительность работы  
   - Останавливает всю сеть, если A или B генерируют событие переполнения  

NodeC фактически является диспетчером и предохранителем всей системы.

### **Запуск вычислений**

Вся сеть активируется отправкой **bang‑события** в **NodeB**, которое вводит начальное значение `1`.  
С этого момента каузальный цикл A → C → B → C → A начинает генерировать последовательность Фибоначчи по модулю заданного предела.


```ocaml
open OUnit2
open Instructions

let make_vm () =
  Vm.create ~stack_capacity:100 ~max_steps:100

let pp_list lst =
  "[" ^ (String.concat "; " (List.map string_of_int lst)) ^ "]"
  

(* ------------------------------------------------------------ *)
(* Test: Fibonacci network with AddMod                          *)
(* ------------------------------------------------------------ *)

let test_fibonacci_mod_network _ctx =
  let vm = make_vm () in
  let ceil = 21 in

  (* AddMod program *)
  let addmod_prog = [
    AddMod;
    EmitIfNonZero "overflow"; 
    Pop;
    EmitTo "default";
  ] in
  
  let forward_prog ch_out =
    [
      LogStack;
      HaltIfEq (1, 0);
      EmitTo "default";
      EmitTo ch_out;
      Pop;
      PushConst (-1);
      Add;
      LogStack;
    ]
  in

  (* ------------------------------------------------------------ *)
  (* Node A                                                       *)
  (* ------------------------------------------------------------ *)
  let bA = Builder.Node.create ~state:[0; ceil] ~vm in
  let inA = bA.add_handler addmod_prog in
  let outA = bA.add_out_port "default" in
  let outA_overflow = bA.add_out_port "overflow" in
  let nodeA = bA.finalize () in

  (* ------------------------------------------------------------ *)
  (* Node B                                                       *)
  (* ------------------------------------------------------------ *)
  let bB = Builder.Node.create ~state:[0; ceil] ~vm in
  let inB = bB.add_handler addmod_prog in
  let outB = bB.add_out_port "default" in
  let outB_overflow = bB.add_out_port "overflow" in
  let nodeB = bB.finalize () in

  (* ------------------------------------------------------------ *)
  (* Node C                                                       *)
  (* ------------------------------------------------------------ *)
  let limit = 10 in
  let bC = Builder.Node.create ~state:[limit] ~vm in

  let inC_ch1 = bC.add_handler (forward_prog "ch1_out") in
  let inC_ch2 = bC.add_handler (forward_prog "ch2_out") in
  let inC_overflow = bC.add_handler [Halt] in

  let outC = bC.add_out_port "default" in
  let outC_ch1 = bC.add_out_port "ch1_out" in
  let outC_ch2 = bC.add_out_port "ch2_out" in

  let nodeC = bC.finalize () in

  (* ------------------------------------------------------------ *)
  (* Build network using Builder.Net + DSL wiring operator        *)
  (* ------------------------------------------------------------ *)
  let nb, ( --> ) = Builder.Net.create () in

  let idA = nb.add_node nodeA in
  let idB = nb.add_node nodeB in
  let idC = nb.add_node nodeC in

  (* Wiring *)
  (idA, outA) --> (idC, inC_ch1);
  (idC, outC_ch1) --> (idB, inB);
  (idB, outB) --> (idC, inC_ch2);
  (idC, outC_ch2) --> (idA, inA);

  (* Overflow wiring *)
  (idB, outB_overflow) --> (idC, inC_overflow);
  (idA, outA_overflow) --> (idC, inC_overflow);

  let net = nb.finalize () in

  (* ------------------------------------------------------------ *)
  (* Run simulation                                               *)
  (* ------------------------------------------------------------ *)
  let init_snap = Runtime.create ~lifespan:30 net in

  let digest =
    Runtime.run
      ~bang:{ dst = idB; in_port_id = inB; payload = 1 }
      init_snap
  in

  let res_stream =
    Digest.node_out_stream_on_port ~node_id:idC ~out_port:outC digest
  in
  
  assert_equal [1; 1; 2; 3; 5; 8; 13] res_stream;

  Printf.printf "Total steps: %d\n" (Digest.total_steps digest.history);
  Printf.printf "NodeC emitted values: %s\n" (pp_list res_stream)


(* ------------------------------------------------------------ *)

let suite =
  "runtime tests" >::: [
    "test fibonacci modulo" >:: test_fibonacci_mod_network;
  ]

let () = run_test_tt_main suite
```
### Running this produces:

```ocaml
1 1 2 3 5 8 13
```
A Fibonacci sequence emerges from simple local rules and causal wiring.

# 📦 Installation
sh
opam install emu
# 🧪 Running the example
sh
dune exec test_fib_mod
# 📚 License
MIT (or your license)

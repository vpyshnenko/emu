...
let stp_init = [
  ...
  PushConst (-1);
  StoreTo mem.parent_node_id;                 (* RAM[parent_node_id] <- -1 *)
  Pop;                                      (* Clean up the stack *)
  
  (* Root's distance to itself is 0 *)
  PushConst 0;
  StoreTo mem.distance;                       (* RAM[distance] <- 0 *)
...
let stp = [
  ...
  LoadTo mem.distance;                        (* Stack: [ current_distance; proposed_distance; proposed_distance ] *)
  ...
  BranchOf [|
    ...
    [
      ...
      StoreTo mem.parent_node_id;             (* RAM[parent_node_id] <- sender_id *)
      Pop;                                  (* Clean up stack *)
      
      (* Update our stored distance to the new, shorter distance *)
      StoreTo mem.distance;                   (* RAM[distance] <- proposed_distance *)
...
let count = [
  ...
  BranchOf [|
    ...
    [ 
      ...
      LoadTo mem.parent_node_id;              (* Load our parent's ID to address the packet *)
	  PopToOut;

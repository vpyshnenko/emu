...
type observer_input = {
  ...
  value : int;    (* Port to receive auth success notifications *)
}
...
type observer_output = {
  ...
  value : int;    (* Output port for auth success (to external) *)
}
...
let make_observer () : observer =
  ...
  let value = b.add_handler [
    EmitTo 2;           (* ReEmit to port 1 (value output) *)
  ] in
  
  (* OUTPUT PORTS - in declaration order *)
  let setup_ok_out = b.add_out_port () in   (* port 0 *)
  let auth_fail_out = b.add_out_port () in  (* port 1 *)
  let value_out = b.add_out_port () in  (* port 2 *)

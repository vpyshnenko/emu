...
let make_net ~l () : t =
  ...
  let connect_lt (a: Router.t)  (b: Router.t) = 
    ...
    (a.id, a.output.flush_lt) --> (b.id, b.input.flush);
    (b.id, b.output.flush_complete) --> (a.id, a.input.flush_lt_complete);
    (a.id, a.output.reset) --> (b.id, b.input.reset)
  in
  	
  let connect_gt (a: Router.t) (b: Router.t) = 
    (a.id, a.output.data_gt) --> (b.id, b.input.data);
    (a.id, a.output.flush_gt) --> (b.id, b.input.flush);
    (b.id, b.output.flush_complete) --> (a.id, a.input.flush_gt_complete);
    (a.id, a.output.reset) --> (b.id, b.input.reset)
  in
  ...
  let sink = Sink.make ~id:(next_id ()) in
  ...
  (ext.id, ext.output.flush) --> (root_router.id, root_router.input.flush);
